...
namespace cuda_wmma {
    ...
    __global__ void __launch_bounds__((BN / WMMA_N) * WARPSIZE * (BM / WMMA_M)) recompute_Weff_re_grad_re_kernel(
        ...
        float*       grad_re_out,     // [B, N]
        __nv_bfloat16* A_t_temp,      // [B, A_dim]
        __nv_bfloat16* grad_W_eff,    // [B, N, N]
        float        alpha,
        int          N,               // N
        int          a_dim,
        int          seq_len,
    ...
    __global__ void __launch_bounds__(128) compute_grad_r_kernel_vec(
       ...
       int batch_size)
    {
        ...
        float* s_grad_re = smem;

        int b = blockIdx.x;
        int i = (blockIdx.y * blockDim.x + threadIdx.x) * 4;

        if (b >= batch_size || i >= N) return;

        for (int idx = threadIdx.x * 4; idx < N; idx += blockDim.x * 4) {
            *reinterpret_cast<float4*>(&s_grad_re[idx]) = *reinterpret_cast<const float4*>(&grad_re[b * N + idx]);
        }
        ...
        for (int j = 0; j < N; ++j) {
            float4 w = *reinterpret_cast<const float4*>(&W_eff[b * N * N + j * N + i]);
            float g = s_grad_re[j];
        ...
        float4 old_grad_r = *reinterpret_cast<const float4*>(&grad_r[b * N + i]);
        float4 result;
        result.x = sum4.x + old_grad_r.x * (1.0f - alpha);
        result.y = sum4.y + old_grad_r.y * (1.0f - alpha);
        result.z = sum4.z + old_grad_r.z * (1.0f - alpha);
        result.w = sum4.w + old_grad_r.w * (1.0f - alpha);

        *reinterpret_cast<float4*>(&grad_r[b * N + i]) = result;
    }
    ...
    )
{
    ...
    for (int t = seq_len - 1; t >= 0; --t) {
        ...
        // -------------------------------------------------------------
        dim3 block(128);
        dim3 grid(batch_size, (N / 4 + block.x - 1) / block.x);
        size_t smem_size = N * sizeof(float);
        compute_grad_r_kernel_vec<<<grid, block, smem_size>>>(
