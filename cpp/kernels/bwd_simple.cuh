...
__global__ void recompute_Weff_re_kernel(
    ...
    float J0,                     // scalar (for demo – replace with matrix if needed)
    float J1,                     // scalar
    const float* Wo,              // [N, N]
    ...
    int batch_size)
{
    ...
    for (int i = threadIdx.x; i < N; i += blockDim.x) {
        r_prev[i] = bump_history[b * (seq_len + 1) * N + t * N + i];
    }
    __syncthreads();
    ...
    int batch_size)
{
    ...
    for (int n = lane; n < N; n += blockDim.x) {
        ...
        float go = grad_output[b * seq_len * N + t * N + n];
    ...
    int batch_size)
{
    ...
    IndexWrapper<const float, 3> bump_idx(bump_history, batch_size, seq_len + 1, N);
    ...
    for (int idx = threadIdx.x; idx < N * N; idx += blockDim.x) {
        ...
        grad_W_eff_idx.at(b, i, j) = grad_re_idx.at(b, i) * bump_idx.at(b, t, j);
    }
}
    ...
    int batch_size)
{
    ...
    for (int t = seq_len - 1; t >= 0; --t) {
        ...
        recompute_Weff_re_kernel<ACT, BM, BN, BK,
                                 WMMA_M, WMMA_N, WMMA_K>
            <<<grid1, block1, shmem_bytes>>>(
                A, Wa, J0, J1, Wo,
                bump_history,
                W_eff_temp, re_temp,
